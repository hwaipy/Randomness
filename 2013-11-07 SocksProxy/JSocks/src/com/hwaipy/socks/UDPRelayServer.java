package com.hwaipy.socks;

import com.hwaipy.socks.server.ServerAuthenticator;
import java.net.*;
import java.io.*;
...
class UDPRelayServer implements Runnable {
     ...
     * port.
     *
     * @param clientIP Address of the client from whom datagrams will be
     * recieved and to whom they will be forwarded.
     * @param clientPort Clients port.
     * @param master_thread Thread which will be interrupted, when UDP relay
     * server stoppes for some reason.
     * @param controlConnection Socket which will be closed, before interrupting
     * the master thread, it is introduced due to a bug in windows JVM which
     * does not throw InterruptedIOException in threads which block in I/O
     * operation.
     */
