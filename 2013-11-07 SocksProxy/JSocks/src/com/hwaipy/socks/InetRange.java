...
public class InetRange implements Cloneable {
    ...
    public synchronized void add(InetAddress from, InetAddress to) {
        all.addElement(new Object[]{from.getHostAddress() + ":" + to.getHostAddress(), null, new Long(ip2long(from)),
            new Long(ip2long(to))});
    }
     ...
     *
     * @param host Host name to check.
     * @param attemptResolve Wether to lookup ip address which corresponds to
...
/////////////////
    ...
    private boolean checkHostEnding(String host) {
        ...
        while (eEnum.hasMoreElements()) {
            if (host.endsWith((String) eEnum.nextElement())) {
                return true;
            }
        }
        return false;
