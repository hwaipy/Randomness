...
public class Socks5DatagramSocket extends DatagramSocket {
     ...
     * CProxy.setDefaultProxy() method. If default proxy is not set or it is set
     ...
     * proxy, the one set with CProxy.setDefaultProxy() method. If default proxy
     ...
     * default proxy, the one set with CProxy.setDefaultProxy() method. If
    ...
    public Socks5DatagramSocket(CProxy p, int port, InetAddress ip)
        ...
        super(port, ip);
        if (p == null) {
            throw new SocksException(CProxy.SOCKS_NO_PROXY);
        }
     ...
     * <p>
     * CProxy might decide to stop forwarding datagrams, in which case it should
