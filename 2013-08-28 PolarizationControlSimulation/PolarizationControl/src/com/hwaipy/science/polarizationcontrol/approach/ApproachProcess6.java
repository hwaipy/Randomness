...
public class ApproachProcess6 {
    ...
    public ApproachProcess6.ApproachResult approach() {
        logging(0, "Start Approching", false);
        logging(0, "", true);
        int left = rotate();
        logging(0, "End Approching", false);
        logging(0, "", true);
        return new ApproachProcess6.ApproachResult(left > 0,
    ...
    private int rotateCollective(int index, int contrast, int rotationCountMax) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            recordWavePlatesStatus();
            wavePlates[1 - index].increase(Math.PI / 8);
            rotationCountMax = doRotateCollective(index, false, contrast, rotationCountMax);
        }
        return rotationCountMax;
    }
    ...
    /**
     *
     * Total: 10000 Success: 7604 MeanStep: 305.6421620199895
     *
     *
     * @param args
     */
    public static void main(String[] args) {
        ApproachProcess6 p = new ApproachProcess6();
        p.loggingLevel = -1;
        int count = 0;
        int overallCount = 0;
        int successCount = 0;
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            ApproachProcess6.ApproachResult result = p.approach();
            overallCount++;
            if (result.success) {
                successCount++;
