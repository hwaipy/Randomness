...
public class ApproachProcessContrast {
  ...
  private final double stepLength = 0.01 / 180 * Math.PI;
  private final int maxStepsCount = 10000;
  private final double innerThreshold = 0.95;
  private final double outerThreshold = 1.05;
  ...
  public ApproachProcessContrast(double[] angles) {
    random = new Random();
    wavePlates[0] = new WavePlate(Math.PI / 2, angles[0]);
    wavePlates[1] = new WavePlate(Math.PI / 2, angles[1]);
    wavePlates[2] = new WavePlate(Math.PI, angles[2]);
  }
...
//    System.out.println(wavePlates[0].getTheta() / Math.PI * 180 + "," + wavePlates[1].getTheta() / Math.PI * 180 + "," + wavePlates[2].getTheta() / Math.PI * 180 + ",");
    ...
    while (rotationCountMax > 0) {
      if (contrastH() > threshold && contrastD() > threshold) {
        break;
      }
...
//    System.out.println(sb.toString());
  ...
  public class ApproachResult {
    ...
    public ApproachResult(boolean success,
            ...
            double cH, double cD, int stepCount, double[] angles) {
  ...
  public static void main(String[] args) {
    ApproachProcessContrast p = new ApproachProcessContrast(new double[]{0.9, 0.4, 0.9});
    p.loggingLevel = -10;
    p.setFiberTransform(0.9, 0.4, 0.7);
