 ...
 * @author Hwaipy
 */
public class ConsolePhaseCompIntegratedWithNoRotateAbove {
...
//    args = new String[]{"../../../实验计划/" + dateS + "/偏振补偿/偏振补偿.xls"};
    ...
    angles.stream().forEach((angle) -> {
      ...
      M1Simulation.M1SimulationResult result = s.calculate(rH, rV, phase / 180 * Math.PI, rotate / 180 * Math.PI, false, false, AES / 180 * Math.PI);
  ...
  private static LinkedList<double[]> loadAngles(File file) throws IOException {
    ...
    for (int i = 1; i <= sheet.getLastRowNum(); i++) {
      ...
      double AESSignal = row.getCell(10).getNumericCellValue();
      double AES = row.getCell(4).getNumericCellValue() - row.getCell(3).getNumericCellValue();
      angles.add(new double[]{A, E, AS, phase, HWP, AES * AESSignal});
    }
    return angles;
  }
}
