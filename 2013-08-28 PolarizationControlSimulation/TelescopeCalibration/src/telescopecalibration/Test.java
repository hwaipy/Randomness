...
//        double[] comp = isMT ? new double[]{80.1, 127.5, 61.4} : new double[]{115.3, -89.2, 138.2};//2016.10.2
        double[] comp = isMT ? new double[]{78.968, 127.045, 60.765} : new double[]{111.716, -95.458, 136.858};//2017.3.1
...
//        double[] measurements = calculateMeasurements(fibreMatrix);
        double[] aoWP1 = calculateAngleOfWP(measurements);

        AnnealingTest p = new AnnealingTest(0.99999, 4048, compLogic, aoWP1);
        AnnealingTest.Result result = p.process();
        if (!result.success) {
            throw new RuntimeException();
        }
        double[] aoWP = result.angles;

        System.out.println(HalfWavePlate.create(aoWP[2]).getMatrix()
                .times(QuarterWavePlate.create(aoWP[1]).getMatrix())
                .times(QuarterWavePlate.create(aoWP[0]).getMatrix())
...
//        Matrix<Complex> output = transform(input, fibreMatrix, qwp1.getMatrix(), qwp2.getMatrix(), hwp.getMatrix());
