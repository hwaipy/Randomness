...
public class ConsolePhaseCompIntegrated {
    ...
    public static void main(String[] args) throws IOException {
        ...
        double Q1 = Double.parseDouble(properties.getProperty("Q1"));
        double Q2 = Double.parseDouble(properties.getProperty("Q2"));
        double H3 = Double.parseDouble(properties.getProperty("H3"));
        double P1 = Double.parseDouble(properties.getProperty("P1"));
        double P2 = Double.parseDouble(properties.getProperty("P2"));
        double P3 = Double.parseDouble(properties.getProperty("P3"));
        double AE = Double.parseDouble(properties.getProperty("AS"));
        String dateS = LocalDateTime.now().minusHours(8).format(DateTimeFormatter.ofPattern("yyyyMMdd"));
//        dateS = "20170408";
//        args = new String[]{"/Users/Hwaipy/Google Drive/LabWork/工程/2016-07-21 丽江站建设/8.每日实验数据/" + dateS + "/偏振补偿/偏振补偿.xls"};
        ...
        angles.stream().forEach((angle) -> {
            double rH = angle[0];
            double rV = angle[1];
            double rotate = angle[2] * AE;
            double phase = angle[3];
            boolean tomoQWP = angle[5] > 0.5;
            M1Simulation s = new M1Simulation();
            M1Simulation.M1SimulationResult result = s.calculate(rH, rV, phase / 180 * Math.PI, rotate / 180 * Math.PI, false, tomoQWP, 0.0 / 180 * Math.PI, P1, P2, P3);
        ...
        try (PrintWriter printWriter = new PrintWriter(outputFile)) {
            for (int i = 1; i < results.size(); i++) {
                ...
                double a1 = angle[0] / Math.PI * 180 + Q1;
                double a2 = angle[1] / Math.PI * 180 + Q2;
                double a3 = angle[2] / Math.PI * 180 + H3;
