...
//        traversal();
    ...
    public static void traversal() {
        ...
        double stepLength = 22.5 / 180 * Math.PI;
        for (double qwp1A = start; qwp1A <= end; qwp1A += stepLength) {
            for (double qwp2A = start; qwp2A <= end; qwp2A += stepLength) {
                for (double hwp = start; hwp <= end; hwp += stepLength) {
                    ...
                    double[] powers = calidM1Process.readPower(Polarization.D);
...
//        System.out.println();
    ...
    private char fitStock(double power) {
        if (Math.abs(power - stocksH[0]) < 0.001) {
            return 'H';
        }
        if (Math.abs(power - stocksH[1]) < 0.001) {
            return 'V';
        }
        if (Math.abs(power - stocksH[2]) < 0.001) {
            return 'D';
        }
        if (Math.abs(power - stocksH[3]) < 0.001) {
            return 'A';
        }
        if (Math.abs(power - stocksH[4]) < 0.001) {
            return 'L';
        }
        if (Math.abs(power - stocksH[5]) < 0.001) {
            return 'R';
        }
        return '?';
    }
